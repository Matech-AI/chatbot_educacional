...
const AuthVerificationPage = () => {
  ...
  useEffect(() => {
    ...
    // No componente AuthVerificationPage, após verificação bem-sucedida
    
    // Dentro do useEffect que verifica o token
    useEffect(() => {
      const verifyToken = async () => {
        try {
          ...
          if (response.ok) {
            setStatus('success');
            setMessage('Conta verificada com sucesso! Uma senha temporária foi enviada para seu e-mail.');
            // Não redirecionamos automaticamente, deixamos o usuário clicar no botão para ir para o login
          } else {
            setStatus('error');
            setMessage(
