/*
  ...
  1. Tables
    ...
    - assistant_templates (reusable assistant configurations)
...
-- Profiles table with secure reference to auth.users
...
-- Secure the reference to auth.users after table creation
DO $$
BEGIN
  IF NOT EXISTS (
...
CREATE POLICY "Admins and instructors can update own materials"
  ...
  USING (
    EXISTS (
      ...
      WHERE profiles.id = auth.uid()
      AND profiles.role IN ('admin', 'instructor')
      AND (materials.uploaded_by = auth.uid() OR profiles.role = 'admin')
    )
  );
