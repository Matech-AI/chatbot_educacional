...
class VideoContentAnalyzer:
    ...
    def __init__(self, openai_api_key: str):
        ...
        self.analysis_cache = {}
        self.cache_file = Path("video_analysis_cache.json")
        self._load_cache()
    ...
    def _load_cache(self):
        ...
        if self.cache_file.exists():
            try:
                ...
                logger.info(f"Loaded video analysis cache with {len(self.analysis_cache)} entries")
        ...
        if any(word in filename_lower for word in ['combinar', 'treino', 'aeróbio', 'força']):
            base_timestamps = [
                ...
                VideoTimestamp(
                    ...
                    keywords=["evidências", "estudos", "pesquisa", "científico"],
                    confidence=0.8
                ),
                VideoTimestamp(
                    ...
                    keywords=["prática", "aplicação", "treino", "implementação"],
                    confidence=0.7
                ),
                VideoTimestamp(
    ...
    def find_topic_timestamp(self, video_timestamps: List[VideoTimestamp], topic_query: str) -> Optional[VideoTimestamp]:
        ...
        for timestamp in video_timestamps:
            ...
            keyword_matches = sum(1 for keyword in timestamp.keywords if keyword.lower() in topic_query_lower)
            score += (keyword_matches / len(timestamp.keywords)) * 0.3 if timestamp.keywords else 0
...
class DriveVideoHandler:
    ...
    def get_video_metadata(self, file_path: str, drive_id: str = None) -> VideoMetadata:
        ...
        metadata = VideoMetadata(
            ...
            updated_at=datetime.fromtimestamp(file_path.stat().st_mtime) if file_path.exists() else datetime.now(),
        ...
        if self.content_analyzer:
            try:
                timestamps = self.content_analyzer.analyze_video_content(str(file_path))
                metadata.timestamps = [
                    {
                        "start_time": ts.start_time,
                        "end_time": ts.end_time,
                ...
                # Generate summary
                metadata.summary = self._generate_video_summary(timestamps, metadata.title)
    ...
    def _generate_video_summary(self, timestamps: List[VideoTimestamp], title: str) -> str:
        ...
        duration_minutes = max(ts.end_time for ts in timestamps) / 60 if timestamps else 0
    ...
    def find_video_for_topic(self, topic_query: str, video_files: List[str]) -> Optional[Tuple[str, float]]:
        ...
        for video_path in video_files:
            ...
            try:
                ...
                timestamps = self.content_analyzer.analyze_video_content(video_path)
                
                # Find matching timestamp
                matching_timestamp = self.content_analyzer.find_topic_timestamp(timestamps, topic_query)
    ...
    def get_video_embed_data(self, video_path: str, topic_query: str = None, drive_id: str = None) -> Dict[str, Any]:
        ...
        if embed_url and start_time > 0:
            embed_url = self.generate_secure_embed_url(drive_id or "", start_time)
        ...
        if handler.content_analyzer:
            ...
            for query in test_queries:
                ...
                if result:
                    ...
                    print(f"Query '{query}': Found at {timestamp:.0f}s in {Path(video_file).name}")
                else:
